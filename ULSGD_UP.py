...
class EnhancedCluster:
    """支持多维输入的自适应聚类算法"""
    def __init__(self):
        self.model = OPTICS(min_samples= 5, cluster_method='xi')
...
class CommunicationTracker_2:
    ...
    def record_communication(self, comm_time, param_size, total_time):
        self.comm_time += comm_time
        self.overall_time.append(total_time)
...
# 添加数据到训练数据集中
    ...
    def train(self):
        if self.method_name == 'sgd':
            ...
            self.theta, self.v = bm.Momentum_sgd(self.X, self.y, 5, 0.1, 10, 0.9, self.v, self.theta)
        elif self.method_name == 'A_sgd':
            self.theta, self.m_t, self.v_t = bm.Adam_sgd(self.X, self.y, 5, 0.1, 5, 0.9, 0.999, self.theta, self.m_t, self.v_t)
            ...
            labels = self.cluster_model.fit_predict(self.X)
            print("聚类标签:", labels)
        elif self.method_name == 'EnhancedCluster':
            self.cluster_model = EnhancedCluster()
            self.cluster_model.fit_predict(self.X.reshape(-1, 1))
        elif self.method_name == 'Net':
            self.model = bm.Net(tau_max - tau_min)
            self.train_dataloader = DataLoader(self.train_dataset, batch_size=batch_size, shuffle=True, drop_last=True)
            bm.train_net(self.model, self.train_dataloader, 0.05, 50)
    ...
    def predict(self, x):
        ...
        try:
            ...
            if self.method_name == 'Knn':
                ...
                c_x = np.array(self.X).reshape(-1, 4)
                new_x = np.array(x).reshape(-1, 4)
                combined_data = np.vstack((c_x, new_x))
                label = int(self.cluster_model.fit_predict(combined_data)[-1])
                val = label + 5 if label != -1 else 4
                rel = min(8,val)
                return rel
            elif self.method_name == 'Net':
                print(x)
                data_np = np.array(x).reshape(-1,4)
                data_np = np.tile(data_np, (24, 1))
            ...
            elif self.method_name == 'sgd':
                x.reshape(-1,4)
                print(np.dot(self.theta, x))
                print(150*np.dot(self.theta, x))
                rel = max(4, int(150*np.dot(self.theta, x)))
                rel = min(8,rel)
                return rel
            elif self.method_name == 'Syn_sgd':
                x.reshape(-1,4)
                val = np.dot(self.theta, x)
                val = val if val > 0 else -val
                print(val)
                print(55*val)
                rel = max(4, int(55*val))#250
                rel = min(8,rel)
                return rel
            elif self.method_name == 'Local_syn_sgd':
                x.reshape(-1,4)
...
class MNISTModel(nn.Module):
    ...
    def record_metrics(self, phase, loss, acc, tau):
        ...
        self.new_metrics[f'{phase}_tau'].append(tau)
    ...
    def run(self):
        ...
        for _ in range(total_epoch):
            ...
            for step in range(self.tau):
                ...
                self.model.record_metrics('train', loss, correct/total, 0)
                self.model.record_metrics('val', val_loss, val_acc, 0)
                self.model.train_metrics['epoch_time'].append(compute_end - compute_start)
                if (step + 1) % self.tau == 0:
                    self.model.record_metrics('tau', self.tau, self.tau, self.tau )
                    comm_start = time.perf_counter()
                    with self.sync.lock:
                        self.sync.worker_updates.append((self.worker_id, self.gradient, step, self.timestamps[step]['compute'] / self.tau, self.timestamps[step]['compute']))
                        ...
                        com_times = [item[4] for item in self.sync.worker_updates[-self.sync.num_workers:]]
                        latest_times_2d = np.array(latest_times).reshape(-1, 1)
                        print(latest_times_2d.shape)
                        if(train_self == 0):
                            predicted_tau = self.tau_model.predict(latest_times_2d.reshape(-1,4))
                            current_tau = max(1, int(predicted_tau))
                            self.tau_model.update_training_set(np.array(latest_times_2d).reshape(-1,4), np.array([current_tau]))
                            self.tau_model.train()
                            train_self += 1
                            self.tau = current_tau 
                            self.tau_history.append(current_tau)
                            print(f'The train_self is {train_self}')
                            print(f'worker_{self.worker_id} fit at {self.tau}')
                        else:
                            self.tau_history.append(current_tau)
                            train_self +=1
                            self.tau = current_tau
                    ...
                    overall_times = [com_time + comm_end - comm_start for com_time in com_times]
                    total_params = sum(p.numel() for p in global_model.parameters())
                    self.sync.comt.record_communication(comm_end - comm_start, total_params, overall_times)
...
def local_with_4w(method='SVM'):
    ...
    for i in range(len(workers[0].model.train_metrics["train_acc"])):
        ...
        global_model.record_metrics('train', train_loss, train_acc,0)
        global_model.record_metrics('val', val_loss, val_acc,0)
        global_model.train_metrics['epoch_time'].append(epoch_time)
    
    over_tau = 0
    for i in range(len(workers[0].model.new_metrics["tau_tau"])):
        over_tau += workers[0].model.new_metrics["tau_tau"][i]
    global_model.over = over_tau
