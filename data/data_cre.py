...
class Tau_model_nom:
    ...
    def train(self):
        if self.method_name == 'sgd':
            ...
            self.theta, self.v = bm.M_sgd(self.X, self.y, 5, 0.1, 10, 0.01, self.v, self.theta)
        elif self.method_name == 'A_sgd':
            self.theta, self.m_t, self.v_t = bm.A_sgd(self.X, self.y, 5, 0.1, 10, 0.1, 0.1, self.theta, self.m_t, self.v_t)
