...
def value_strenthen(output_path, if_str, length):
    ...
    if not if_str:
        ...
        for _ in range(length):
            new_row = {}
            for name in ['workers_1', 'workers_2', 'workers_3', 'workers_4']:
                rand = np.random.normal(loc=data[name][0], scale=np.sqrt(data[name][1]))
...
def find_loss(args):
    ...
    for i in range(tau_min + 1, tau_max + 1):
        loss = np.max(i * value_list) - np.min(i * value_list)
        if min_loss >= loss:
            min_loss = loss
            index = i
...
def tau_predicter(row):
    ...
    return find_loss(workers)

def target_append(output_path):
    df = pd.read_csv(output_path + '/mid_output.csv', names=['workers_1', 'workers_2', 'workers_3', 'workers_4'], header=None)
    df['tau'] = df.apply(tau_predicter, axis=1)
    df.to_csv(output_path + '/final_output.csv', index=False)

def data_loading_sgd_lin(target_path):
    data_tracker(target_path)
    

    value_strenthen(target_path, stren, stren_length)
    value_correction(target_path)
    
    data_preparation(target_path)
    target_append(target_path)
...
class CustomDataset(Dataset):
    ...
    # 使用 .iloc 明确按位置访问
        ...
        label = pd.to_numeric(self.data.iloc[idx, -1], errors='coerce')
